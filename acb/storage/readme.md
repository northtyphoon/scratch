...
```bash
az storage account create --name mystorage --resource-group MyResourceGroup --default-action deny --bypass AzureServices
```

# Create an ACR Task with system-assigned identity

```bash
az acr task create -r MyRegistry -n MyTask -f acb.yaml -c /dev/null --assign-identity
```

# Assign 'Storage Blob Data Contributor' role to the system-assigned idenity
...
```bash
az acr task run -r MyRegistry -n MyTask --set account=mystorage
```